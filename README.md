...
> **A note on search previews:** Because our local search index is powered by the community-maintained [manami-project Anime Offline Database](https://github.com/manami-project/anime-offline-database), search results for AniDB-sourced titles will show a poster and title, but **not** a description — manami's dataset doesn't include synopses. For rarer titles that aren't in manami at all but do exist in the official AniDB XML data dump (often more niche/adult content), search results will show **neither** a poster nor a description — just the title. **This only affects what you see while searching.** Once you actually add a series, Anidarr fetches the real, complete metadata — poster, description, full season and episode data, alternate titles, the works — so every series in your library ends up fully filled out regardless of how sparse it looked in the search results.
