...
Why rely on just one source? Anidarr supports **AniDB** as the primary metadata provider. You can easily fetch the absolute best metadata for your collection without relying on error-prone community-maintained mapping databases.
...
Once installed, head over to **Settings > Metadata** to configure your AniDB credentials. The UI is custom-tailored to link directly to AniDB pages instead of TVDB.
