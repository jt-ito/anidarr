...
namespace Sonarr.Api.V3.Series
{
    ...
    public class SeriesLookupController : Controller
    {
        ...
        public SeriesLookupController(ISearchForNewSeries searchProxy, IMetadataDispatcher metadataDispatcher, IBuildFileNames fileNameBuilder, IMapCoversToLocal coverMapper)
        {
        ...
        [HttpGet]
        public IEnumerable<SeriesResource> Search([FromQuery] string term, [FromQuery] string provider = null)
        {
            IEnumerable<NzbDrone.Core.Tv.Series> results;

            // ponytail: just pass provider if it exists
            if (!string.IsNullOrWhiteSpace(provider) && global::System.Enum.TryParse<MetadataProviderType>(provider, true, out var providerType))
            {
